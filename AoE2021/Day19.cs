...
namespace AoE2021
{
	public class Day19 : Day
    {
        ...
        protected override object FirstTask()
		{
			...
			var coordinatesPerScan = new Dictionary<string, List<Coordinates>>();
			var scanners = new List<Coordinates>();
			scanners.Add(new(0, 0, 0));
			...
			foreach (var scan in scans)
			{
				...
				coordinatesPerScan.Add(scan[0], list);
			}
			while (coordinatesPerScan.Count > 1)
			{

				var pointsPerScan = new Dictionary<string, List<Point>>();
				foreach (var scan in coordinatesPerScan)
				{
					var points = new List<Point>();
					foreach (var point in scan.Value)
					{
						var distances = new List<Distance>();
						foreach (var point2 in scan.Value.Where(x => x != point))
						{
							distances.Add(point.GetDistance(point2));
						}
						points.Add(new(point, distances));
					}
					pointsPerScan[scan.Key] = points;
				}

				var s0 = pointsPerScan.First(x => x.Key.Contains('0'));

				foreach (var s1 in pointsPerScan.Skip(1))
				{
					...
					if (mainScanP1 != null)
					{
						...
						scanners.Add(new(xDiff, yDiff, zDiff));
						...
						coordinatesPerScan[s0.Key] = all.ToList();
						coordinatesPerScan.Remove(s1.Key);
						break;
					}
				}
			}

			return coordinatesPerScan.First().Value.Count();
		}
		...
		protected override object SecondTask()
		{
			...
			var manhattan = new List<int>();
			foreach (var scanner in scanners)
			{
				foreach (var scanner2 in scanners)
				{
					var distance = Math.Abs(scanner.X - scanner2.X) + Math.Abs(scanner.Y - scanner2.Y) + Math.Abs(scanner.Z - scanner2.Z);
					manhattan.Add(distance);
				}
			}

			return manhattan.Max();
		}
